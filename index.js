    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        try {
            ...
            if (lower === '.antilink on') {
                antilinkOn = true;
                await sock.sendMessage(from, { text: '✅ Anti-Link ON Strict! Admin er tao delete hobe bos!' });
                return;
            }
            ...
            if (lower === '.ping') {
                await sock.sendMessage(from, { text: '✅ Bot Active bos!' });
                return;
            }
            ...
            if (!botIsAdmin) {
                await sock.sendMessage(from, { text: '❌ Amake Admin dao bos!' });
                return;
            }
...
app.get('/', (req, res) => {
    ...
    if (isConnected) {
        html += '<h2 style="color:#25D366;">CONNECTED!</h2><p>Anti-Link Strict ON ✅</p>';
    } else if (qrImage && qrImage.startsWith('data:')) {
        html += '<img src="' + qrImage + '"><p>Scan QR Bos</p>';
    } else {
        html += '<p>Loading QR... Refresh after 30s</p>';
    }
    html += '<br><button onclick="location.reload()" style="padding:8px 15px;background:#25D366;border:none;border-radius:6px;font-weight:bold">REFRESH</button></div><script>setTimeout(()=>location.reload(),25000)</script></body></html>';
    res.send(html);
});
